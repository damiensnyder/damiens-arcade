...
import { parse as parse$2, serialize } from 'cookie';
...
function get_cookies(request, url) {
  ...
  const initial_cookies = parse$2(header, { decode: (value) => value });
  let normalized_url;
  ...
  const cookies = {
    ...
    get(name, opts) {
      ...
      const req_cookies = parse$2(header, { decode: opts?.decode });
     ...
     */
    getAll(opts) {
      const cookies2 = parse$2(header, { decode: opts?.decode });
  ...
  function get_cookie_header(destination, header2) {
    ...
    if (header2) {
      const parsed = parse$2(header2, { decode: (value) => value });
