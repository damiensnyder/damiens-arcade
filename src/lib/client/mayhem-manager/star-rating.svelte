<script lang="ts">
	let { rating, oldRating = -1 }: { rating: number; oldRating?: number } = $props();

	if (oldRating === -1) {
		oldRating = rating;
	}

	let starSize = $derived.by(() => {
		if (typeof window === 'undefined') return '18px';
		return window.innerWidth > 720 ? (window.innerWidth > 1200 ? '20px' : '18px') : '15px';
	});

	const starSvg =
		'M5 7.457 L8.09 9.51 L7.092 5.937 L10 3.632 L6.293 3.477 L5 0 L3.707 3.477 L0 3.632 L2.908 5.937 L1.91 9.51 L5 7.457 Z';
