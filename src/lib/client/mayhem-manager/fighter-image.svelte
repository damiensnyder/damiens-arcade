<script lang="ts">
	import { type Equipment, EquipmentSlot } from '$lib/shared/mayhem-manager/types';

	let { fighter, equipment, inBattle = false }: { fighter: any; equipment: Equipment[]; inBattle?: boolean } = $props();

	let imageSize = $derived.by(() => {
		if (typeof window === 'undefined') return '12rem';
		return window.innerWidth > 720 ? (window.innerWidth > 1200 ? '15rem' : '12rem') : '10rem';
	});
...
>
	...
	{:else}
		<img
			src={fighter.appearance.shorts}
			class="equipment legs"
