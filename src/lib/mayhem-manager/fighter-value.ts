...
const coeffsRaw = readFileSync("src/lib/mayhem-manager/data/ai-coeffs.csv").toString();
...
export function fighterValueInBattle(fighter: Fighter, equipment: Equipment[]): number {
  ...
  for (const c of Object.keys(COEFFICIENTS)) {
    if (c.includes(":")) {
      const [left, right] = c.split(":");
      value += COEFFICIENTS[c] * weightOfCoeff(left, fighter, equipment) * weightOfCoeff(right, fighter, equipment);
    } else {
